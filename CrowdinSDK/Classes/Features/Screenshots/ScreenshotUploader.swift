...
class CrowdinScreenshotUploader: ScreenshotUploader {
	...
	func loginAndGetProjectId(success: (() -> Void)? = nil, errorHandler: ((Error) -> Void)? = nil) {
        if CrowdinLogin.isLogined {
            self.getProjectId(success: success, errorHandler: errorHandler)
        } else if let crowdinLogin = CrowdinLogin.shared {
            crowdinLogin.login(completion: {
                self.getProjectId(success: success, errorHandler: errorHandler)
            }) { err in
                errorHandler?(err)
            }
	...
	func getProjectId(success: (() -> Void)? = nil, errorHandler: ((Error) -> Void)? = nil) {
        let distrinbutionsAPI = DistributionsAPI(hashString: hash, organizationName: organizationName, auth: CrowdinLogin.shared)
	...
	func uploadScreenshot(screenshot: UIImage, controlsInformation: [ControlInformation], name: String, success: (() -> Void)?, errorHandler: ((Error) -> Void)?) {
		...
		let screenshotsAPI = ScreenshotsAPI(organizationName: organizationName, auth: CrowdinLogin.shared)
        let storageAPI = StorageAPI(organizationName: organizationName, auth: CrowdinLogin.shared)
